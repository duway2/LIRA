...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-full flex bg-gray-50">
      {/* Sidebar Navigation */}
      <aside className="w-64 flex-shrink-0 bg-white border-r border-gray-200 shadow-sm flex flex-col justify-between hidden md:flex sticky top-20 h-[calc(100vh-80px)] overflow-y-auto">
        <div>
          ...
          <nav className="px-4 space-y-2 mt-4">
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm border ${
                ...
                pathname.startsWith("/dashboard/articles/")
                  ? "bg-red-50 text-lira-red border-red-100 shadow-sm"
                  : "text-gray-600 hover:text-lira-red hover:bg-red-50 border-transparent"
              } ${!isOnboardingComplete ? "opacity-50 pointer-events-none" : ""}`}
            >
              <svg
                className="w-5 h-5 opacity-80"
                fill="none"
            ...
            <Link
              ...
              aria-disabled={!isOnboardingComplete}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm border mt-4 ${
