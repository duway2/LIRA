...
export default function Navbar() {
  ...
  // Parse JWT token from cookies on mount
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        // Often JWT contains subject/name or we just show a dashboard fallback
      } catch (e) {
        setIsLoggedIn(false);
      }
    } else {
    ...
    <header className="fixed w-full z-50 bg-white/90 backdrop-blur-md border-b border-gray-200 shadow-sm transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-4 sm:gap-6 text-sm font-medium">
          <Link href="/" className="text-gray-600 hover:text-lira-red transition-colors duration-200 hidden md:block">Tentang</Link>
          <Link href="/berita" className="text-gray-600 hover:text-lira-red transition-colors duration-200 hidden md:block">Berita</Link>
          ...
          {isLoggedIn ? (
            ...
            >
              ...
              {role === "admin" ? "Admin Console" : "Dashboard Akun"}
            </Link>
          ) : (
            <>
              <Link href="/auth/login" className="text-gray-800 hover:text-lira-red transition-colors duration-200 font-bold">Login</Link>
              <Link href="/auth/register" className="bg-lira-red hover:bg-lira-red-light text-white px-5 py-2.5 rounded-full transition-all duration-300 shadow-lg shadow-lira-red/20 font-semibold uppercase tracking-wide text-xs">
                Gabung Member
              </Link>
            </>
          )}
